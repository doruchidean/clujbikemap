...
import com.doruchidean.clujbikemap.helpers.WidgetHelper;
...
public class MapsActivity extends AppCompatActivity
    ...
    private double
            userLatitude = 46.775627,
            userLongitude = 23.590935;
    ...
    private List<String> mBuses;
    private BusSchedule mSelectedBus;
...
//			WindowManager.LayoutParams.FLAG_FULLSCREEN);
    ...
    private void initializeDatabase(){
        ...
        mSelectedBus = databaseHandler.getBusSchedule(PersistenceManager.getBusName(this));

        mBuses = databaseHandler.getAllBuses();
            ...
            if (mSelectedBus != null) {
                if (databaseHandler.isActualized(PersistenceManager.getBusTableUpdatedDay(this))) {
                    ...
                    ApiClient.getInstance()
                            .getBusSchedule(getBusScheduleCallback, mSelectedBus.getBusNumber());
                }
            }
            ...
            ApiClient.getInstance()
                    .getAllBuses(new Callback() {
                        ...
                        @Override public void onResponse(Call call, Response response) throws IOException {
                            databaseHandler.insertBusNames(Factory.getInstance().parseBusNames(response));
                            mBuses = databaseHandler.getAllBuses();
                        }
                    });
        }
    ...
    private Callback getBusScheduleCallback = new Callback() {
        ...
        @Override public void onResponse(Call call, final Response response) throws IOException {
            ...
            if (response.isSuccessful()) {
                Factory.getInstance().factorizeBusSchedule(response, mSelectedBus);
                GeneralHelper.updateDatabase(MapsActivity.this, mSelectedBus);
    ...
    private void updateBusBarUI() {
        ...
        HashMap<String, ArrayList<String>> leavingTimes = new GetBusScheduleForToday(this,
                mSelectedBus.getBusNumber()).loadInBackground();

        if(leavingTimes == null || leavingTimes.isEmpty()) return;

        tvSelectedBus.setText(mSelectedBus.getName());

        plecariCapat1 = leavingTimes.get(Factory.PLECARI_CAPAT_1);
        plecariCapat2 = leavingTimes.get(Factory.PLECARI_CAPAT_2);

        String capat1Title = leavingTimes.get(Factory.NUME_CAPETE).get(0);
        String capat2Title = leavingTimes.get(Factory.NUME_CAPETE).get(1);
    ...
    private void setTextsInBusBar(ArrayList<String> minutesRemaining, LinearLayout parent){
        ...
        if(minutesRemaining.size() == 0){
            tvSubtitle.setText(String.format(getString(R.string.bus_departing_over_limit),
                    GeneralHelper.busLeavingMaxLimit));
            return;
        }
    ...
    public void onClick(View v) {
        ...
        switch(v.getId()){
            ...
            case (R.id.btn_select_bus):
                ...
                final BusListAdapter busListAdapter = new BusListAdapter(MapsActivity.this, mBuses);
                int selectedBusPosition=-1;
                String busName = PersistenceManager.getBusName(this);
                if (!busName.isEmpty()) {
                    for(int i=0; i<mBuses.size();i++){
                        if(mBuses.get(i).equalsIgnoreCase(busName)){
                ...
                lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                    ...
                    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                        String busName = busListAdapter.getItem(position);
                        PersistenceManager.setBusName(MapsActivity.this, busName);
                ...
                builder.setView(busesContainer)
                        ...
                        .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                DatabaseHandler databaseHandler = DatabaseHandler.getInstance(MapsActivity.this);
                                String busName = PersistenceManager.getBusName(MapsActivity.this);

                                mSelectedBus = databaseHandler.getBusSchedule(busName);
                                String plecariLV = mSelectedBus.getPlecariCapat1LV();
                                if (plecariLV != null && !plecariLV.isEmpty()) {
                                    updateBusBarUI();
                                } else {
                                    String busNumber = databaseHandler.getBusNumber(busName);
                                    ApiClient.getInstance().getBusSchedule(getBusScheduleCallback,
                                            busNumber);
                                }
                            }
                        })
